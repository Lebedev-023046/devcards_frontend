...
export function applyTheme(mode: ThemeMode) {
  ...
  Object.entries(tokens.spacing).forEach(([k,v]) => commonVars[`--spacing-${k}`] = v);
  Object.entries(tokens.radius) .forEach(([k,v]) => commonVars[`--radius-${k}`]  = v);
  Object.entries(tokens.shadow) .forEach(([k,v]) => commonVars[`--shadow-${k}`]  = v);
  Object.entries(tokens.font)   .forEach(([k,v]) => commonVars[`--font-${k}`]    = v);
  setVars(commonVars);
